...
public:
    ...
    {
        // uint32_t mortonCode = mortonEncode2D(static_cast<uint16_t>(itemsize.getPosition().x), static_cast<uint16_t>(itemsize.getPosition().y));
...
uint32_t mortonEncode2D(uint16_t x, uint16_t y)
{
    uint32_t answer = 0;
...
struct SearchCache
{
    ...
    std::list<StaticQuadTreeContainer<SomeObjectWithArea>::QuadTreeContainer::iterator> cachedResults;
};
...
int main()
{
    ...
    while (window.isOpen())
    {
            ...
            std::abs(currentView.height - searchCache.previousView.height) > 1.0f)
        {
            ...
            searchCache.cachedResults = quadtree.search(rScreen);
        ...
        m.setSize(vSearchArea);
        m.setPosition(mousePos - vSearchArea / 2.0f);
        window.draw(m);
